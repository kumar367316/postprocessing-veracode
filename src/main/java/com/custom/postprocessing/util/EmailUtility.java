...
public class EmailUtility {
	...
	public MailResponse sendEmail(MailRequest request, Map<String, Object> model, String currentDate) {
		...
		try {
			...
			messageBodyPart.setFileName(documentTxtFile.toString());
	 ...
	 * String currentDate) { MailResponse response = new MailResponse(); Properties
	 * props = new Properties(); props.put(starttlsKey, starttlsValue);
	 * props.put(hostKey, hostValue); props.put(portKey, portValue);
	 * props.put(debugKey, debugValue); props.put(authKey, authValue); Session
	 * session = Session.getDefaultInstance(props); MimeMessage message = new
	 * MimeMessage(session); try { MimeMessageHelper helper = new
